#!/usr/bin/env python
"""
Comprehensive Analysis for NSWPL Matches
- Manly UTD vs Sydney United 58
- Macaroni Stallions vs Sydney FC NPL
...
def analyze_soccer_match(
    ...
    date="2026-06-11", league="Australia_NSWPL"
):
...
def run_manly_utd_vs_sydney_united_58():
    """Run analysis for Manly UTD vs Sydney United 58"""
    
    print("\n" + "=" * 80)
    print("NSWPL MATCH: MANLY UTD vs SYDNEY UNITED 58")
    print("Australia NSW Premier League - June 11, 2026")
    print("=" * 80 + "\n")
    ...
    result = analyze_soccer_match(
        home_team="Manly UTD",
        away_team="Sydney United 58",
        home_data=home_data,
        away_data=away_data,
        market_data=market_data,
        venue="Cromer Park, Sydney",
        date="2026-06-11",
        league="Australia_NSWPL"
    )
    
    return result


def run_macaroni_stallions_vs_sydney_fc_npl():
    """Run analysis for Macaroni Stallions vs Sydney FC NPL"""
    
    print("\n" + "=" * 80)
    print("NSWPL MATCH: MACARONI STALLIONS vs SYDNEY FC NPL")
    print("Australia NSW Premier League - June 11, 2026")
    print("=" * 80 + "\n")
    
    # Macaroni Stallions (Home)
    home_data = {
        'xg_for': 1.30,
        'xg_against': 1.60,
        'shots': 10.5,
    ...
    result = analyze_soccer_match(
        home_team="Macaroni Stallions",
        away_team="Sydney FC NPL",
        home_data=home_data,
        away_data=away_data,
        market_data=market_data,
        venue="Macarthur Stadium, Sydney",
        date="2026-06-11",
        league="Australia_NSWPL"
    )
...
def main():
    """Run both NSWPL match analyses"""
    
    print("=" * 80)
    print("NSWPL MATCHES COMPREHENSIVE ANALYSIS")
    print("June 11, 2026")
    print("=" * 80)
    
    # Run Manly UTD vs Sydney United 58
    manly_sydney_result = run_manly_utd_vs_sydney_united_58()
    
    # Run Macaroni Stallions vs Sydney FC NPL
    macaroni_sydney_result = run_macaroni_stallions_vs_sydney_fc_npl()
    
    # Save results
    output_dir = Path("output")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(output_dir / "manly_utd_vs_sydney_united_58_analysis.json", 'w') as f:
        json.dump(manly_sydney_result, f, indent=2)
    
    with open(output_dir / "macaroni_stallions_vs_sydney_fc_npl_analysis.json", 'w') as f:
        json.dump(macaroni_sydney_result, f, indent=2)
    ...
    print("=" * 80)
    print()
    print("Manly UTD vs Sydney United 58:")
    print(f"  Outcome: {manly_sydney_result['recommendations']['match_outcome']}")
    print(f"  Goals: {manly_sydney_result['recommendations']['goals']}")
    print(f"  BTTS: {manly_sydney_result['recommendations']['btts']}")
    print(f"  Corners: {manly_sydney_result['recommendations']['corners']}")
    print()
    print("Macaroni Stallions vs Sydney FC NPL:")
    print(f"  Outcome: {macaroni_sydney_result['recommendations']['match_outcome']}")
    print(f"  Goals: {macaroni_sydney_result['recommendations']['goals']}")
    print(f"  BTTS: {macaroni_sydney_result['recommendations']['btts']}")
    print(f"  Corners: {macaroni_sydney_result['recommendations']['corners']}")
    print()
    print(f"Results saved to:")
    print(f"  - output/manly_utd_vs_sydney_united_58_analysis.json")
    print(f"  - output/macaroni_stallions_vs_sydney_fc_npl_analysis.json")
    print()
    print("=" * 80)
