#!/usr/bin/env python
"""
Comprehensive Analysis for Netherlands vs Uzbekistan
...
def run_comprehensive_analysis():
    """Run comprehensive analysis for Netherlands vs Uzbekistan"""
    
    print("=" * 80)
    print("COMPREHENSIVE MATCH ANALYSIS: NETHERLANDS vs UZBEKISTAN")
    ...
    home_data = {
        'xg_for': 1.95,           # High - strong attacking
        'xg_against': 1.05,       # Good defense
        'shots': 14.5,            # High shot volume
        'sot': 5.5,               # Good shot quality
        'goals_for': 1.8,         # Strong scoring
        'goals_against': 0.95,    # Good defending
        'clean_sheets_last10': 5, # Good defensive organization
        ...
        'tempo': 0.35,            # Fast tempo
        'width_crossing': 0.70,   # Good width play
        'final_third_pressure': 0.75,  # High pressure in final third
    }
    
    # Away Team: Uzbekistan
    away_data = {
        'xg_for': 1.10,           # Moderate - limited but capable
        'xg_against': 1.55,       # Vulnerable defense
        'shots': 10.0,            # Moderate shot volume
        'sot': 3.5,               # Modest shot quality
        'goals_for': 1.0,         # Moderate scoring
        'goals_against': 1.4,     # Vulnerable defending
        'clean_sheets_last10': 3, # Modest defensive organization
        'missing_attacker': 0,    # Full strength
        'missing_creator': 1,     # Missing creative midfielder
        'missing_cb': 0,          # Defensive unit intact
        'missing_gk': 0,          # Goalkeeper available
        'tempo': 0.0,             # Neutral tempo
        'width_crossing': 0.50,   # Moderate width play
        'final_third_pressure': 0.45,  # Moderate pressure in final third
    }
    ...
    print(f"   Netherlands Goal Strength: {home_goal_strength:.2f}")
    print(f"   Uzbekistan Goal Strength: {away_goal_strength:.2f}")
    print(f"   Offensive Edge: {'Netherlands' if home_goal_strength > away_goal_strength else 'Uzbekistan'}")
    print()
    ...
    print(f"   Netherlands BTTS Strength: {home_btts_strength:.2f}")
    print(f"   Uzbekistan BTTS Strength: {away_btts_strength:.2f}")
    ...
    print(f"   Netherlands Corner Strength: {home_corner_strength:.2f}")
    print(f"   Uzbekistan Corner Strength: {away_corner_strength:.2f}")
    print()
    ...
    print(f"   Netherlands Expected Goals: {home_lam:.2f}")
    print(f"   Uzbekistan Expected Goals: {away_lam:.2f}")
    ...
    print("=" * 80)
    print()
    print(f"   Projected Score: Netherlands {home_lam:.1f} - Uzbekistan {away_lam:.1f}")
    print(f"   Total Expected Goals: {total_lam:.2f}")
    ...
    output_path = Path("output/netherlands_vs_uzbekistan_analysis.json")
