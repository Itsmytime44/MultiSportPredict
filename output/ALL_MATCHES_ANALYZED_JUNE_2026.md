# All Matches Analyzed - June 2026
## MultiSportPredict Model Analysis Log

**Generated:** June 5, 2026  
**Repository:** https://github.com/Itsmytime44/MultiSportPredict.git

---

## Complete List of Matches Analyzed (June 1-5, 2026)

### June 1, 2026 (Monday)

| # | Match | League | Sport | Output File |
|---|-------|--------|-------|-------------|
| 1 | Alba Berlin vs Brose Bamberg | BBL (Germany) | Basketball | `alba_berlin_vs_brose_bamberg_results.json` |
| 2 | European Template Results | Multi-league | Basketball | `european_template_results.json` |
| 3 | Gigantes de Carolina vs Manatí | BSN (Puerto Rico) | Basketball | `gigantes_carolina_vs_manati_results.json` |
| 4 | Luanda Villa vs Mombassa United | FIFA Africa Qualifiers | Soccer | `luanda_villa_vs_mombassa_united_results.json` |

---

### June 2, 2026 (Tuesday)

| # | Match | League | Sport | Output File |
|---|-------|--------|-------|-------------|
| 5 | UCAM Murcia vs Barcelona | ACB (Spain) | Basketball | `murcia_vs_barcelona_analysis.json` |

---

### June 3, 2026 (Wednesday)

| # | Match | League | Sport | Output File |
|---|-------|--------|-------|-------------|
| 6 | Albania vs Israel | FIFA World Cup Qualifiers | Soccer | `albania_vs_israel_analysis.json` |
| 7 | Netherlands vs Algeria | FIFA World Cup Qualifiers | Soccer | `netherlands_vs_algeria_analysis.json` |
| 8 | Poland vs Nigeria | FIFA World Cup Qualifiers | Soccer | `poland_vs_nigeria_analysis.json` |
| 9 | Three Matches Analysis (Combined) | Multi-league | Basketball | `three_matches_analysis.json` |
| 10 | Wales vs Ghana | FIFA World Cup Qualifiers | Soccer | `wales_vs_ghana_analysis.json` |
| 11 | Atlanta Hawks vs NY Giants | Cross-sport Analysis | Basketball vs American Football | `hawks_vs_giants_analysis.json` |
| 12 | The Bay Hawks vs Nelson Giants | NBL (New Zealand) | Basketball | `sydney_vs_nws_analysis.json` |
| 13 | San Antonio Spurs vs NY Knicks | NBA | Basketball | `spurs_vs_knicks_analysis.json` |

---

### June 4, 2026 (Thursday)

| # | Match | League | Sport | Output File |
|---|-------|--------|-------|-------------|
| 14 | Barcelona vs UCAM Murcia | ACB (Spain) | Basketball | `barcelona_vs_murcia_analysis.json` |
| 15 | Telekom Baskets Bonn vs Bayern Munich | BBL (Germany) | Basketball | `bonn_vs_bayern_live_analysis.json` |
| 16 | **Otago Nuggets vs Franklin Bulls** | NZNBL (New Zealand) | **Basketball** | `otago_nuggets_vs_franklin_bulls_analysis.json` |
| 17 | Spain vs Iraq | FIFA World Cup Qualifiers | Soccer | `spain_vs_iraq_analysis.json` |
| 18 | Sweden vs Greece | FIFA World Cup Qualifiers | Soccer | `sweden_vs_greece_analysis.json` |

---

### June 5, 2026 (Friday)

| # | Match | League | Sport | Output File |
|---|-------|--------|-------|-------------|
| 19 | Anadolu Efes vs Fenerbahce | EuroLeague | Basketball | `anadolu_efes_vs_fenerbahce_analysis.json` |
| 20 | Panathinaikos vs Olympiacos | EuroLeague | Basketball | `panathinaikos_vs_olympiacos_analysis.json` |
| 21 | FBK Karlstad vs IF Karlstad | Sweden Division 2 | Soccer | `fbk_karlstad_vs_if_karlstad_analysis.json` |
| 22 | FC Haka vs JaPS | Finland Ykkonen | Soccer | `fc_haka_vs_japs_analysis.json` |
| 23 | KaPa vs KTP Kotka | Finland Ykkonen | Soccer | `kapa_vs_ktp_kotka_analysis.json` |
| 24 | Joventut Badalona vs Baskonia | ACB (Spain) | Basketball | `joventut_baskonia_analysis.json` |
| 25 | Georgia vs Bahrain | FIFA World Cup Qualifiers | Soccer | `georgia_vs_bahrain_analysis.json` |

---

## Summary Statistics

| Category | Count |
|----------|-------|
| **Total Matches Analyzed** | 25 |
| **Basketball Matches** | 14 |
| **Soccer Matches** | 11 |
| **EuroLeague Games** | 2 |
| **ACB (Spain) Games** | 3 |
| **BBL (Germany) Games** | 2 |
| **FIFA World Cup Qualifier Games** | 7 |
| **NZNBL Games** | 1 |
| **Soccer - Sweden** | 1 |
| **Soccer - Finland** | 2 |

---

## Analysis Types Per Match

### Basketball Analysis Includes:
- **1Q (First Quarter)** - Spread, Total, Moneyline
- **1H (First Half)** - Spread, Total, Moneyline
- **FG (Full Game)** - Spread, Total, Moneyline
- **Player Props** - Points, Rebounds, Assists, etc.
- **Efficiency Metrics** - ORTG, DRTG, Pace, Rest/Travel

### Soccer Analysis Includes:
- **Match Outcome** - Home/Draw/Away
- **Goals O/U** - Over/Under 2.5 goals
- **BTTS** - Both Teams To Score
- **Corners O/U** - Over/Under corners
- **Expected Goals (xG)** - Team projections

---

## Key Highlights

### Notable Matches:
1. **Otago Nuggets vs Franklin Bulls** (June 4) - New Zealand NBL
2. **EuroLeague Derbies** (June 5) - Turkish and Greek classics
3. **Barcelona vs Murcia** - Spanish ACB rivalry
4. **Spurs vs Knicks** - NBA matchup
5. **FIBA Qualifiers** - International basketball action

### Sports Covered:
- Basketball (NBA, EuroLeague, ACB, BBL, NBL, FIBA, NBL)
- Soccer (Sweden Division 2, Finland Ykkonen)

---

## Output Files Location
All analysis results are stored in the `output/` directory:
- JSON files contain detailed model projections and recommendations
- `BETTING_RECOMMENDATIONS_LOG.md` contains summarized betting recommendations
- This file provides a complete match listing

---

*This log is automatically generated by the MultiSportPredict system. All matches are analyzed using the European template model for basketball and Poisson distribution model for soccer.*