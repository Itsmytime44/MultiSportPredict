...
def run_greece_italy_analysis():
    """Run analysis for Greece vs Italy"""
    
    print("\n" + "=" * 80)
    print("INTERNATIONAL FRIENDLY: GREECE vs ITALY")
    print("June 7, 2026")
    print("=" * 80 + "\n")
    
    # Greece (Home) - Defensive minded team
    home_data = {
        'xg_for': 1.25,
        'xg_against': 1.15,
        'shots': 10.5,
    ...
    # Italy (Away) - Strong European side
    away_data = {
        'xg_for': 1.70,
        'xg_against': 1.05,
        'shots': 14.5,
...
if __name__ == "__main__":
    result = run_greece_italy_analysis()
    
    # Save results
    output_dir = Path("output/soccer")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(output_dir / "greece_vs_italy_analysis.json", 'w') as f:
        json.dump(result, f, indent=2)
    
    print(f"\nDetailed results saved to: {output_dir / 'greece_vs_italy_analysis.json'}")