#!/usr/bin/env python
"""
Comprehensive Analysis for Spain vs Iraq
...
def run_comprehensive_analysis():
    """Run comprehensive analysis for Spain vs Iraq"""
    
    print("=" * 80)
    print("COMPREHENSIVE MATCH ANALYSIS: SPAIN vs IRAQ")
    ...
    # Home Team: Spain
    home_data = {
        'xg_for': 1.85,           # Expected goals scored per game
        'xg_against': 0.85,       # Expected goals conceded per game
        'shots': 15.5,            # Average shots per game
        'sot': 5.8,               # Average shots on target per game
        'goals_for': 1.7,         # Actual goals scored per game
        'goals_against': 0.8,     # Actual goals conceded per game
        'clean_sheets_last10': 6, # Clean sheets in last 10 games
        ...
        'tempo': 0.35,            # Playing tempo (positive = fast)
        'width_crossing': 0.70,   # Width and crossing tendency (0-1)
        'final_third_pressure': 0.72,  # Pressure in final third (0-1)
    }
    
    # Away Team: Iraq
    away_data = {
        'xg_for': 1.05,           # Expected goals scored per game
        'xg_against': 1.55,       # Expected goals conceded per game
        'shots': 9.5,             # Average shots per game
        'sot': 3.2,               # Average shots on target per game
        'goals_for': 0.9,         # Actual goals scored per game
        'goals_against': 1.5,     # Actual goals conceded per game
        'clean_sheets_last10': 2, # Clean sheets in last 10 games
        'missing_attacker': 1,    # Number of missing attackers
        'missing_creator': 1,     # Number of missing creative midfielders
        'missing_cb': 0,          # Number of missing center backs
        'missing_gk': 0,          # Number of missing goalkeepers
        'tempo': 0.0,             # Playing tempo (positive = fast)
        'width_crossing': 0.40,   # Width and crossing tendency (0-1)
        'final_third_pressure': 0.42,  # Pressure in final third (0-1)
    }
    
    # Market Data (Estimated typical lines for this matchup)
    market_data = {
        'open_line': 2.75,        # Opening total goals line
        'current_line': 2.75,     # Current total goals line
        'total': 2.75,            # Game total
    ...
    print(f"   Spain Goal Strength: {home_goal_strength:.2f}")
    print(f"   Iraq Goal Strength: {away_goal_strength:.2f}")
    print(f"   Offensive Edge: {'Spain' if home_goal_strength > away_goal_strength else 'Iraq'}")
    print()
    ...
    print(f"   Spain BTTS Strength: {home_btts_strength:.2f}")
    print(f"   Iraq BTTS Strength: {away_btts_strength:.2f}")
    ...
    print(f"   Spain Corner Strength: {home_corner_strength:.2f}")
    print(f"   Iraq Corner Strength: {away_corner_strength:.2f}")
    print()
    ...
    print(f"   Spain Expected Goals: {home_lam:.2f}")
    print(f"   Iraq Expected Goals: {away_lam:.2f}")
    ...
    print(f"   Spain Win Probability: {p_home_win:.3f} ({p_home_win*100:.1f}%)")
    print(f"   Draw Probability: {p_draw:.3f} ({p_draw*100:.1f}%)")
    print(f"   Iraq Win Probability: {p_away_win:.3f} ({p_away_win*100:.1f}%)")
    
    # Moneyline recommendation
    if p_home_win >= 0.60:
        ml_lean = "Spain Win"
    elif p_away_win >= 0.35:
        ml_lean = "Spain Win (narrow)"
    else:
        ml_lean = "Spain Win"
    ...
    # Team corners projection (handle negative values)
    h_abs = max(home_corner_strength, 0.5)
    a_abs = max(away_corner_strength, 0.5)
    home_corner_proj = corner_total * (h_abs / (h_abs + a_abs))
    away_corner_proj = corner_total * (a_abs / (h_abs + a_abs))
    
    print(f"   Projected Total Corners: {corner_total:.1f}")
    print(f"   Spain Projected Corners: {home_corner_proj:.1f}")
    print(f"   Iraq Projected Corners: {away_corner_proj:.1f}")
    ...
    print("-" * 40)
    print()
    print("   FACTORS FAVORING SPAIN:")
    print("   [+] Home advantage")
    print(f"   [+] Much better xG for ({home_data['xg_for']:.2f} vs {away_data['xg_for']:.2f})")
    print(f"   [+] Significantly more shots ({home_data['shots']:.1f} vs {away_data['shots']:.1f})")
    print(f"   [+] Superior defensive record ({home_data['goals_against']:.1f} GA vs {away_data['goals_against']:.1f} GA)")
    ...
    print(f"   [+] Much higher corner strength ({home_corner_strength:.2f} vs {away_corner_strength:.2f})")
    print(f"   [+] Higher tempo ({home_data['tempo']:.2f} vs {away_data['tempo']:.2f})")
    print()
    print("   FACTORS FAVORING IRAQ:")
    print("   [+] Counter-attacking potential")
    print("   [+] Spain may dominate possession but leave spaces")
    print()
    print("   FACTORS FAVORING OVER:")
    print(f"   [+] Spain's dominant attacking (xG: {home_data['xg_for']:.2f})")
    print(f"   [+] Total xG suggests {total_lam:.2f} goals")
    print("   [+] Iraq missing attackers and creators")
    print()
    print("   FACTORS FAVORING UNDER:")
    print(f"   [+] Spain's strong defense (xG against: {home_data['xg_against']:.2f})")
    print("   [+] Iraq may park the bus")
    print()
    print("   FACTORS FAVORING BTTS:")
    print("   [+] Spain's high line may allow counters")
    print(f"   [+] Iraq needs to attack ({away_data['xg_for']:.2f} xG)")
    print()
    print("   FACTORS FAVORING SPAIN CORNERS:")
    print(f"   [+] Dominant possession style")
    ...
    print("=" * 80)
    print()
    print(f"   Projected Score: Spain {home_lam:.1f} - Iraq {away_lam:.1f}")
    print(f"   Total Expected Goals: {total_lam:.2f}")
    print(f"   Moneyline: Spain {p_home_win:.1%} | Draw {p_draw:.1%} | Iraq {p_away_win:.1%}")
    print(f"   BTTS Probability: {btts_prob:.1%}")
    print(f"   Projected Corners: {corner_total:.0f}")
    print(f"   Spain Projected Corners: {home_corner_proj:.0f}")
    print(f"   Iraq Projected Corners: {away_corner_proj:.0f}")
    print()
    ...
    # Save to output
    output_path = Path("output/spain_vs_iraq_analysis.json")
