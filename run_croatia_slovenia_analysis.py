...
def run_croatia_slovenia_analysis():
    """Run analysis for Croatia vs Slovenia"""
    
    print("\n" + "=" * 80)
    print("INTERNATIONAL FRIENDLY: CROATIA vs SLOVENIA")
    print("June 7, 2026")
    print("=" * 80 + "\n")
    
    # Croatia (Home) - Stronger team with home advantage
    home_data = {
        'xg_for': 1.75,
        'xg_against': 1.10,
        'shots': 14.0,
    ...
    # Slovenia (Away) - Solid defensive team
    away_data = {
        'xg_for': 1.30,
        'xg_against': 1.35,
        'shots': 11.0,
...
if __name__ == "__main__":
    result = run_croatia_slovenia_analysis()
    
    # Save results
    output_dir = Path("output/soccer")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(output_dir / "croatia_vs_slovenia_analysis.json", 'w') as f:
        json.dump(result, f, indent=2)
    
    print(f"\nDetailed results saved to: {output_dir / 'croatia_vs_slovenia_analysis.json'}")