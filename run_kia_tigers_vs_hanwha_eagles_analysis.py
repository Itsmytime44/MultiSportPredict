...
KBO_CONFIG = {
    ...
    'park_factors': {
        'Gwangju-Kia Champions Field': 1.05,  # Slightly hitter-friendly
...
def get_team_stats(team: str) -> Dict[str, Any]:
    ...
    """
    # Hanwha Eagles (Home) - Strong offensive team, solid pitching at home
    if team in ["HAN", "Hanwha Eagles", "Hanwha", "Eagles"]:
        return {
            ...
            'bullpen_era': 3.95,
            'recent_form': 0.60,  # Win % in last 10
            'home_record': 0.65,  # Home win %
            'vs_opponent_record': 0.58,  # Season series record
        }
    # Kia Tigers (Away) - Weaker team on the road, struggling pitching
    elif team in ["KIA", "Kia Tigers", "Kia"]:
        return {
            ...
            'era': 5.10,
            'whip': 1.48,
            'quality_start_pct': 0.32,
            'bullpen_era': 4.85,
            'recent_form': 0.42,  # Win % in last 10
            'away_record': 0.38,  # Away win %
            'vs_opponent_record': 0.42,  # Season series record
        }
    else:
        # Default stats
        return {
...
def analyze_kbo_match(
    ...
    venue: str = "Gwangju-Kia Champions Field",
    date: str = "2026-06-09",
) -> Dict[str, Any]:
    """
...
def main():
    """Run KBO match analysis for Hanwha Eagles vs Kia Tigers"""
    ...
    # Run analysis - Hanwha Eagles is HOME team (-1.5 favorites)
    ...
    output_path = output_dir / "kia_tigers_vs_hanwha_eagles_analysis.json"
