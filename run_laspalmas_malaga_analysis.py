...
- UD Las Palmas vs Malaga
Spanish League - June 7, 2026
...
def analyze_soccer_match(
    ...
    date="2026-06-07", league="Spain_Segunda_Division"
):
...
def run_laspalmas_malaga_analysis():
    """Run analysis for UD Las Palmas vs Malaga"""
    
    print("\n" + "=" * 80)
    print("SPANISH SEGUNDA DIVISION: UD LAS PALMAS vs MALAGA")
    print("June 7, 2026")
    print("=" * 80 + "\n")
    
    # UD Las Palmas (Home) - Island team with possession style
    home_data = {
        'xg_for': 1.50,
        'xg_against': 1.25,
        'shots': 12.0,
    ...
    # Malaga (Away) - Experienced Segunda side
    away_data = {
        'xg_for': 1.35,
        'xg_against': 1.40,
        'shots': 11.0,
...
if __name__ == "__main__":
    result = run_laspalmas_malaga_analysis()
    
    # Save results
    output_dir = Path("output/soccer")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(output_dir / "laspalmas_vs_malaga_analysis.json", 'w') as f:
        json.dump(result, f, indent=2)
    
    print(f"\nDetailed results saved to: {output_dir / 'laspalmas_vs_malaga_analysis.json'}")