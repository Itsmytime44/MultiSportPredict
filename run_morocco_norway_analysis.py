...
- Morocco vs Norway
International Friendly - June 7, 2026
...
def analyze_soccer_match(
    ...
    date="2026-06-07", league="International_Friendly"
):
...
def run_morocco_norway_analysis():
    """Run analysis for Morocco vs Norway"""
    
    print("\n" + "=" * 80)
    print("INTERNATIONAL FRIENDLY: MOROCCO vs NORWAY")
    print("June 7, 2026")
    print("=" * 80 + "\n")
    
    # Morocco (Home) - Strong African team with good home record
    home_data = {
        'xg_for': 1.55,
        'xg_against': 1.20,
        'shots': 12.5,
    ...
    # Norway (Away) - Physical team with Haaland-type threat
    away_data = {
        'xg_for': 1.65,
        'xg_against': 1.30,
        'shots': 13.0,
...
if __name__ == "__main__":
    result = run_morocco_norway_analysis()
    
    # Save results
    output_dir = Path("output/soccer")
    output_dir.mkdir(parents=True, exist_ok=True)
    
    with open(output_dir / "morocco_vs_norway_analysis.json", 'w') as f:
        json.dump(result, f, indent=2)
    
    print(f"\nDetailed results saved to: {output_dir / 'morocco_vs_norway_analysis.json'}")