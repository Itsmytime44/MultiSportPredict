#!/usr/bin/env python
"""
Comprehensive Analysis for Barcelona vs Isla De Tenerif Canarias
Spanish Liga ACB - June 9, 2026
...
def analyze_basketball_match(
    ...
    date="2026-06-09", league="Spanish Liga ACB"
):
...
def run_barcelona_vs_isla_tenerif():
    """Run comprehensive analysis for Barcelona vs Isla De Tenerif Canarias"""
    
    print("=" * 80)
    print("COMPREHENSIVE ANALYSIS: BARCELONA vs ISLA DE TENERIF CANARIAS")
    print("Spanish Liga ACB - June 9, 2026")
    print("=" * 80)
    print()
    
    # Define team data based on ACB Liga metrics
    # Barcelona (Home) - Top ACB team at home
    home_data = {
        'ortg': 116.8,        # Offensive Rating
        'drtg': 107.5,        # Defensive Rating
        'baseline_net': 9.3,  # Baseline net rating
        'recent_net': 8.7,    # Recent net rating (last 10 games)
        'pace': 75.2,         # Pace (possessions per 40 min - FIBA)
        'rest_days': 4,
        'travel_km': 0,       # Home team, no travel
        'back_to_back': False,
        'three_in_six': False,
        'split_edge': 4.8,    # Home court advantage factor (strong at Palau)
        'rotation_depth': 10, # Number of regular rotation players
        'injury_status': 'green',  # Healthy roster
        'coach_stability': 'green',
        'motivation': 'green',  # Fighting for ACB title
    }
    
    # Isla De Tenerif Canarias (Away) - Mid-table ACB team
    away_data = {
        'ortg': 109.5,        # Offensive Rating
        'drtg': 111.2,        # Defensive Rating
        'baseline_net': -1.7, # Baseline net rating
        'recent_net': -1.2,   # Recent net rating (last 10 games)
        'pace': 73.5,         # Pace (possessions per 40 min - FIBA)
        'rest_days': 3,
        'travel_km': 2100,    # Travel from Tenerife to Barcelona (long flight)
        'back_to_back': False,
        'three_in_six': False,
        ...
        'coach_stability': 'green',
        'motivation': 'yellow',  # Mid-table, less motivation
    }
    
    # Market data
    market_data = {
        'open_line': -7.5,    # Barcelona opened as 7.5 point favorites
        'current_line': -8.5, # Current line moved to Barcelona -8.5
        'spread': -8.5,       # Barcelona favored by 8.5
        'total': 161.5,       # Over/Under total
    }
    
    # Run analysis
    result = analyze_basketball_match(
        home_team="Barcelona",
        away_team="Isla De Tenerif Canarias",
        home_data=home_data,
        away_data=away_data,
        market_data=market_data,
    ...
    # Define player props data for key players (ACB Liga)
    player_props = [
        # Barcelona Players
        {
            "player_name": "Nikola Mirotic",
            "team": "Barcelona",
            "opponent": "Isla De Tenerif Canarias",
            "prop_type": "Points",
            "prop_line": 17.5,
            "player_avg": 18.2,
            "minutes_proj": 28.0,
        ...
        # Isla De Tenerif Canarias Players
        {
            "player_name": "Marcelo Huertas",
            "team": "Isla De Tenerif Canarias",
            "opponent": "Barcelona",
            "prop_type": "Points",
            "prop_line": 12.5,
            "player_avg": 13.2,
            ...
            "opp_position_def_rating": 106.0,  # Barcelona defends well
            "injury_boost": "green",
            "blowout_risk": "yellow",
            "role": "starter",
            "open_prop_line": 12.0,
    ...
    props_results = []
    print(f"{'Player':<25} {'Prop':<15} {'Line':>6} {'Proj':>6} {'Edge':>6} {'Lean':<30}")
    print("-" * 90)
    ...
    for prop in player_props:
        ...
        print(f"{prop['player_name']:<25} {prop['prop_type']:<15} {prop['prop_line']:>6.1f} {proj_value:>6.1f} {edge:>+6.2f} {lean:<30}")
    ...
    output_path = output_dir / "barcelona_vs_isla_tenerif_analysis.json"
