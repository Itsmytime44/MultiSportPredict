#!/usr/bin/env python
"""
Comprehensive KBO Analysis - June 16, 2026
===========================================
Matchup 1: NC Dinos vs Hanwha Eagles (Hanwha Life Eagles Park)
Matchup 2: SSG Landers vs Lotte Giants (Sajik Baseball Stadium)

Running through MultiSportPredict pipeline with sabermetric data,
...
# ============================================================================
# KBO TEAM STATS (Updated with analysis data for June 2026)
# ============================================================================
...
KBO_TEAM_PROFILES = {
    "NC Dinos": {
        ...
        "wrc_plus": 88,
        "swstr_pct": 0.10,
        "notes": "Average offensive production. Bullpen has been inconsistent. Decent contact rates but lacks power. Struggling on the road."
    },
    "Hanwha Eagles": {
        "abbreviation": "HAN",
        "runs_per_game": 4.2,
        ...
        "wrc_plus": 78,
        "swstr_pct": 0.11,
        "notes": "Severe offensive slump. ISO <.120, O-Swing% spiked. Chasing bad pitches. Severe power outage. Pitching staff also struggling."
    },
    "SSG Landers": {
        "abbreviation": "SSG",
        "runs_per_game": 5.3,
        ...
        "wrc_plus": 102,
        "swstr_pct": 0.09,
        "notes": "Solid all-around club. Above-average offense with decent power. Starting pitching has been consistent. Bullpen is reliable."
    },
    "Lotte Giants": {
        "abbreviation": "LOT",
        "runs_per_game": 5.0,
        ...
        "wrc_plus": 85,
        "swstr_pct": 0.105,
        "notes": "Inconsistent offense. Pitching staff has struggled with command (elevated BB/9). Bullpen is shaky. Average home team."
    },
}
...
PARK_FACTORS = {
    "Hanwha Life Eagles Park": {
        ...
        "hits_factor": 1.01,
        "k_factor": 0.99,
        "description": "Neutral-to-slightly offensive park. Average dimensions, moderate run environment."
    },
    "Sajik Baseball Stadium": {
        ...
        "hits_factor": 1.01,
        "k_factor": 0.98,
        "description": "Slightly hitter-friendly park in Busan. Moderate offensive boost."
    }
}
...
def main():
    ...
    # ──────────────────────────────────────────────────────────────────────
    # MATCHUP 1: NC Dinos (Away) @ Hanwha Eagles (Home)
    # Venue: Hanwha Life Eagles Park (Daejeon)
    # ──────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 100)
    print("MATCHUP 1/2: NC Dinos @ Hanwha Eagles")
    print("=" * 100)

    match1_analysis = {
        "sharp_lean": "Under (Total Runs)",
        "prop_angle": "Hanwha Starting Pitcher - Over Strikeouts (NC chase rate elevated, SwStr% above league avg)",
        "park": "Hanwha Life Eagles Park - Neutral park, slightly favoring pitchers on contact suppression",
        "series_context": "Opening game of series. Hanwha looking to snap offensive slump at home.",
        "hittersEdge": "NC has slight offensive edge (5.0 R/G vs 4.2 R/G). Hanwha ISO <.120 severe power outage.",
        "pitchingEdge": "Both staffs have been below average. NC ERA 4.85 vs HAN ERA 4.50. Hanwha bullpen slightly better.",
        "sharpReasoning": "Syndicates leaning Under due to Hanwha's dead offense. NC also struggling on road (42% away record). Low-scoring affair expected.",
    }
    ...
    match1_result = analyze_matchup(
        ...
        date="2026-06-16",
        series_score="First game of series",
        analysis_data=match1_analysis,
    )

    # Save to file
    out_path = output_dir / "nc_dinos_vs_hanwha_eagles_june16_analysis.json"
    with open(out_path, 'w') as f:
        json.dump(match1_result, f, indent=2, default=str)
    print(f"\nResults saved to: {out_path}")

    # ──────────────────────────────────────────────────────────────────────
    # MATCHUP 2: SSG Landers (Away) @ Lotte Giants (Home)
    # Venue: Sajik Baseball Stadium (Busan)
    # ──────────────────────────────────────────────────────────────────────
    print("\n\n" + "=" * 100)
    print("MATCHUP 2/2: SSG Landers @ Lotte Giants")
    print("=" * 100)

    match2_analysis = {
        "sharp_lean": "SSG Landers ML (Moneyline)",
        "prop_angle": "SSG Top Hitter - Over Total Bases (Lotte bullpen struggling, high BB/9 leads to hard contact in scoring positions)",
        "park": "Sajik Baseball Stadium - Slightly hitter-friendly park in Busan",
        "series_context": "Opening game of series. SSG has strong form, Lotte struggling with pitching consistency.",
        "clashOfStyles": "SSG = balanced offense with good power. Lotte = inconsistent contact, shaky bullpen.",
        "ssgAdvantage": "SSG has clear pitching edge (4.30 ERA vs 4.90, 1.35 WHIP vs 1.42). Bullpen also stronger (3.90 vs 4.50 FIP).",
        "lotteWeakness": "Lotte's elevated BB/9 (3.5) combined with SSG's patient approach (.342 OBP) spells trouble. Free passes kill.",
        "sharpReasoning": "Sharp money sides with SSG on the road. Lotte's pitching staff in poor form, giving up too many walks. SSG's balanced attack should find success at Sajik.",
    }
    ...
    # Save to file
    out_path = output_dir / "ssg_landers_vs_lotte_giants_june16_analysis.json"
    with open(out_path, 'w') as f:
    ...
    # ──────────────────────────────────────────────────────────────────────
    print("\n\n" + "=" * 100)
    print("KBO JUNE 16, 2026 - MULTI-SPORT PREDICT ANALYSIS SUMMARY")
    print("=" * 100)
    print()
    m1 = match1_result
    m2 = match2_result
    print(f"MATCH 1: {m1['away_team']['name']} @ {m1['home_team']['name']}")
    ...
    print(f"    TOTAL: {m1['totals']['model_total_lean']} (Conf: {m1['totals']['total_confidence']:.1f}%)")
    print(f"    SIDE: {m1['side_analysis']['lean']} (Conf: {m1['side_analysis']['confidence']:.1f}%)")
    ...
    print(f"    Win Prob: {m2['away_team']['name']} {m2['win_probability']['away_prob']:.1%} | {m2['home_team']['name']} {m2['win_probability']['home_prob']:.1%}")
    print(f"    TOTAL: {m2['totals']['model_total_lean']} (Conf: {m2['totals']['total_confidence']:.1f}%)")
    print(f"    SIDE: {m2['side_analysis']['lean']} (Conf: {m2['side_analysis']['confidence']:.1f}%)")
